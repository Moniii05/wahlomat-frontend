...
export class Profile implements OnInit {
  ...
  noCandidacyWarning = signal(false); // Warnung, wenn keine Kandidatur vorhanden ist
...
// Mehrere Backend-Anfragen parallel ausführen
 ...
 private onFacultyChangedCleanup(): void {
  ...
  removeRow(c: Candidacy): void {
    ...
    if(this.candidacies().length === 0) {
      this.noCandidacyWarning.set(true);
    }
    if (c.candidacyId) {
 ...
 saveAll(): void {
  ...
  // Prüfung: Mindestens eine Kandidatur erforderlich
  if (this.candidacies().length === 0) {
    this.errorMsg.set('Bitte füge mindestens eine Kandidatur hinzu.');
    return;
  }
  ...
  this.profileApi.saveProfile(profileDto).pipe(
    ...
    // SCHRITT 1: Erst alle DELETES
    switchMap(() => {
      if (toDelete.length === 0) return of(null);

      const deleteOps = toDelete.map(id => this.candidacyApi.deleteCandidacy(id));
      return forkJoin(deleteOps);
    }),

    // SCHRITT 2: Kurze Pause nach DELETE
    switchMap(() => {
    ...
    // SCHRITT 3: Dann alle CREATES
    switchMap(() => {
      if (toCreate.length === 0) return of(null);

      const createOps = toCreate.map(c =>
        this.candidacyApi.createCandidacy({
    ...
    // SCHRITT 4: Optional Passwort ändern
    switchMap(() => {
      if (this.isPasswordChangeRequested()) {
    ...
    // SCHRITT 5: Frische Kandidaturen laden
    ...
    next: (fresh) => {
      ...
      // Erfolgsmeldung bestimmen
      const message = this.isPasswordChangeRequested()
        ? 'Profil, Kandidaturen und Passwort erfolgreich gespeichert.'
        : 'Profil & Kandidaturen erfolgreich gespeichert.';
  ...
  // Einheitliches Erfolgs-Feedback
  private handleSuccess(message: string) {
    this.infoMsg.set(message);
    this.errorMsg.set(null); // Fehlermeldung löschen bei Erfolg
  }
